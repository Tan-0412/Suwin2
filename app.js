...
function openPrint(title, today, bodyHtml) {
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    tr.grand-total td,tfoot tr td{background:#fef6e4!important;font-weight:700;font-size:11pt;padding-top:10px;padding-bottom:10px;border-top:2px solid #f0c060;color:#5a3e00;}
    ...
    .dsc-table tr.grand-total td{background:#fef6e4!important;font-weight:700;font-size:11pt;padding-top:10px;padding-bottom:10px;border-top:2px solid #f0c060;color:#5a3e00;}
...
function printReleaseReportFromList(releaseList, rDate) {
  ...
  <div class="doc-header">
    ...
    <div><div class="doc-title">รายงานการปล่อยรถ</div><div class="doc-company">บริษัท อีซูซุคิงส์ยนต์กรุงเทพ จำกัด</div></div>
  </div>
...
function renderSummary(type, scList, curRows, prevRows, allRows, srcCols, fixedSrc, modelCols, monthLabel, today, curYM) {
  ...
  const modelBkHdr=modelCols.length?modelCols.map(m=>`<th class="th-bk" style="font-weight:600;font-size:9px;color:#000;">${m}</th>`).join(''):'<th class="th-bk" style="color:#000;">—</th>';
  const modelRsHdr=modelCols.length?modelCols.map(m=>`<th class="th-rs" style="font-weight:600;font-size:9px;color:#000;">${m}</th>`).join(''):'<th class="th-rs" style="color:#000;">—</th>';
  const srcHdr=srcCols.map(s=>`<th class="th-src" style="font-weight:600;font-size:10px;color:#000;">${s}</th>`).join('');
  ...
  let html=`<table class="sum-table" style="font-size:11px;"><colgroup>${colgroupCols}</colgroup><thead><tr>
    <th rowspan="2" class="th-carry" style="text-align:left;color:#000;">SC</th>
    <th rowspan="2" class="th-carry" style="color:#000;">ยกมา</th>
    <th colspan="${mLen+1}" class="th-bk" style="color:#000;">จองเดือนนี้</th>
    <th colspan="${sLen}" class="th-src" style="color:#000;">แหล่งที่มา (จองเดือนนี้)</th>
    <th rowspan="2" class="th-total" style="color:#000;">รวมจอง<br>ปัจจุบัน</th>
    <th colspan="${mLen+1}" class="th-rs" style="color:#000;">ปล่อยเดือนนี้</th>
    <th rowspan="2" class="th-cl" style="color:#000;">ถอนจอง</th>
  </tr><tr>${modelBkHdr}<th class="th-bk bk-total-col" style="color:#000;font-weight:800;">รวมจอง</th>${srcHdr}${modelRsHdr}<th class="th-rs rs-total-col" style="color:#000;font-weight:800;">รวมปล่อย</th></tr></thead><tbody>`;
  ...
  Object.entries(teams).forEach(([team, members]) => {
    ...
    members.forEach(sc => {
      ...
      for (let i=0;i<maxRows;i++) {
        ...
        const rowBg=rowIdx%2===1?'background:#eeeeee;':'background:#ffffff;';
        rowIdx++;
